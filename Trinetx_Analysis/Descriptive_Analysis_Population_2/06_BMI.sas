...
quit;           /* it should be the same as "37643 - BS users" - yes, it is! */
...
/************************************************************************************
	STEP 3. bmi_bf_glp1 | the clostest value prior to the first glp1 prescription date
************************************************************************************/
...
From min.glp1_users_6335 a inner join min.bs_glp1_bmi_v02 b on (a.patient_id =b.patient_id and b.bmi_date < a.glp1_initiation_date);
Quit;

* latest bmi 6mon prior glp1 initiation date, only have date in the output;
Proc sql;
Create table bs_bmi_glp1_prior_latest as
Select distinct patient_id, glp1_initiation_date, max(bmi_date) as latest_bmi_date_bf_glp1 format=yymmdd10.
From min.bs_glp1_bmi_glp1_prior
Group by patient_id, glp1_initiation_date;
Quit;  

* left join latest BMI date with bmi value;
Proc sql;
Create table min.bs_glp1_bmi_bfglp1 as
Select distinct i.* , p.bmi
From bs_bmi_glp1_prior_latest i left join min.bs_glp1_bmi_glp1_prior p on (i.patient_id = p.patient_id and i.glp1_initiation_date=p.glp1_initiation_date and i.latest_bmi_date_bf_glp1 = p.bmi_date);
Quit;

* calculate median BMI value before glp1 initiation; 
proc means data=min.bs_glp1_bmi_bfglp1 n nmiss median p25 p75;
  var bmi;
  title "bmi before glp1 initiation in GLP1 users";
run;

/************************************************************************************
	STEP 4. To compare BMI trend for GLP-1 users vs non-users, make variables for BMI value over time from BS date
************************************************************************************/
...
from min.glp1_users_6335 a inner join min.bs_glp1_bmi_glp1_prior b on (a.patient_id = b.patient_id and b.bmi_date < a.glp1_initiation_date);
quit;   /* 85488 obs */
