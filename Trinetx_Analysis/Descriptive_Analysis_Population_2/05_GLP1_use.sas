...
data min.glp1_users_6335;
  set min.bs_glp1_user_v03;
  if temporality = 2;
run; /* 6335 */

proc means data=min.glp1_users_6335
...
* add calender year;
data min.glp1_users_6335_v01;
	set min.glp1_users_6335;
 	format glp1_init_year 4.;
	glp1_init_year = year(glp1_initiation_date);
run;
proc print data=min.glp1_users_6335_v01 (obs=30);
	var patient_id glp1_initiation_date glp1_init_year ;
	where glp1_user =1;
run;

/* plotting purpose */
proc freq data=min.glp1_users_6335_v01 noprint;
    tables Molecule*glp1_init_year / out=min.glp1_users_6335_v01_pct;
run;
...
proc sql;
    ...
    from min.glp1_users_6335_v01_pct
